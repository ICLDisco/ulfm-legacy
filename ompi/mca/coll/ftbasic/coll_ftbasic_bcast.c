 ...
 */
int
mca_coll_ftbasic_bcast_lin_intra(void *buff, int count,
                               ...
                               mca_coll_base_module_t *module)
{
    ...
    int err;
    ompi_request_t **preq;
    mca_coll_ftbasic_module_t *basic_module = (mca_coll_ftbasic_module_t*) module;
    ...
    mca_coll_ftbasic_free_reqs(reqs, i);
 ...
 */
int
mca_coll_ftbasic_bcast_log_intra(void *buff, int count,
                               struct ompi_datatype_t *datatype, int root,
                               struct ompi_communicator_t *comm,
                               mca_coll_base_module_t *module)
{
    ...
    for (i = hibit + 1, mask = 1 << i; i <= dim; ++i, mask <<= 1) {
        ...
        if (peer < size) {
            ...
            err = MCA_PML_CALL(isend_init(buff, count, datatype, peer,
                                          ...
                                          comm, preq++));
            if (MPI_SUCCESS != err) {
                mca_coll_ftbasic_free_reqs(reqs, nreqs);
                return err;
            }
        }
    }
 ...
 */
int
mca_coll_ftbasic_bcast_lin_inter(void *buff, int count,
                               struct ompi_datatype_t *datatype, int root,
                               struct ompi_communicator_t *comm,
                               mca_coll_base_module_t *module)
{
 ...
 */
int
mca_coll_ftbasic_bcast_log_inter(void *buff, int count,
                               struct ompi_datatype_t *datatype, int root,
                               struct ompi_communicator_t *comm,
                               mca_coll_base_module_t *module)
{
