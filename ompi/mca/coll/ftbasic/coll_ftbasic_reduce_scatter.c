 ...
 */
int
mca_coll_ftbasic_reduce_scatter_intra(void *sbuf, void *rbuf, int *rcounts,
                                    ...
                                    mca_coll_base_module_t *module)
{
    ...
    if ((op->o_flags & OMPI_OP_FLAGS_COMMUTE) &&
        ...
        /* temporary receive buffer.  See coll_ftbasic_reduce.c for details on sizing */
        ...
        if (0 == rank) {
            /* temporary receive buffer.  See coll_ftbasic_reduce.c for
               details on sizing */
            recv_buf_free = (char*) malloc(buf_size);
            recv_buf = recv_buf_free - lb;
            if (NULL == recv_buf_free) {
 ...
 */
int
mca_coll_ftbasic_reduce_scatter_inter(void *sbuf, void *rbuf, int *rcounts,
                                    struct ompi_datatype_t *dtype,
                                    struct ompi_op_t *op,
                                    struct ompi_communicator_t *comm,
                                    mca_coll_base_module_t *module)
