...
static int rts_ft_event(int state);
#endif

orte_routed_module_t orte_routed_rts_module = {
    init,
    finalize,
    delete_route,
    update_route,
...
static int delete_route(orte_process_name_t *proc)
{
    ...
    OPAL_OUTPUT_VERBOSE((1, orte_routed_base_output,
                         "%s routed_rts_delete_route for %s",
...
{ 
    ...
    if (ORTE_JOB_FAMILY(target->jobid) != ORTE_JOB_FAMILY(ORTE_PROC_MY_NAME->jobid)) {
        ...
        OPAL_OUTPUT_VERBOSE((1, orte_routed_base_output,
                             "%s routed_rts_update: diff job family routing job %s --> %s",
                             ORTE_NAME_PRINT(ORTE_PROC_MY_NAME),
        ...
        for (i=0; i < orte_routed_jobfams.size; i++) {
            ...
            if (jfam->job_family == jfamily) {
                OPAL_OUTPUT_VERBOSE((2, orte_routed_base_output,
                                     "%s routed_rts: updating route to %s via %s",
                                     ORTE_NAME_PRINT(ORTE_PROC_MY_NAME),
        ...
        OPAL_OUTPUT_VERBOSE((2, orte_routed_base_output,
                             "%s routed_rts: adding route to %s",
                             ORTE_NAME_PRINT(ORTE_PROC_MY_NAME),
                             ORTE_JOB_FAMILY_PRINT(target->jobid)));
...
static orte_process_name_t get_route(orte_process_name_t *target)
{
    ...
    if (ORTE_PROC_MY_NAME->vpid == daemon.vpid) {
        ...
        /* Should really check to make sure that the process
         * is still alive before returning the ok.
         */
        for (item = opal_list_get_first(&orte_local_children);
             item != opal_list_get_end(&orte_local_children);
             item = opal_list_get_next(item)) {
            child = (orte_odls_child_t*)item;
            if (child->name->jobid == target->jobid &&
                child->name->vpid  == target->vpid) {
                if( child->state <= ORTE_PROC_STATE_UNTERMINATED ) {
                    ret = target;
                }
                /* The 'CALLED_ABORT' case is special since the process is
                 * waiting for us to tell it that it's ok to abort.
                 */
                else if( child->state == ORTE_PROC_STATE_CALLED_ABORT) {
                    ret = target;
                } else {
                    ret = ORTE_NAME_INVALID;
                }
        ...
        if (ORTE_PROC_IS_HNP) {
            ...
            for(i = 0; i < jdata->procs->size; ++i) {
                ...
                OPAL_OUTPUT_VERBOSE((5, orte_routed_base_output,
                                     "%s routed_rts_get: Checking process %15s state 0x%x",
                                     ORTE_NAME_PRINT(ORTE_PROC_MY_NAME),
...
static int init_routes(orte_jobid_t job, opal_buffer_t *ndat)
{
    /* the rts module routes all proc communications through
    ...
    if (ORTE_PROC_IS_TOOL) {
        OPAL_OUTPUT_VERBOSE((2, orte_routed_base_output,
                             "%s routed_rts: init routes for TOOL job %s\n\thnp_uri %s\n\tdaemon uri %s",
    ...
    if (ORTE_PROC_IS_DAEMON) {
        ...
        OPAL_OUTPUT_VERBOSE((2, orte_routed_base_output,
                             "%s routed_rts: init routes for daemon job %s\n\thnp_uri %s",
                             ORTE_NAME_PRINT(ORTE_PROC_MY_NAME),
        ...
        OPAL_OUTPUT_VERBOSE((2, orte_routed_base_output,
                             "%s routed_rts: completed init routes",
                             ORTE_NAME_PRINT(ORTE_PROC_MY_NAME)));
    ...
    if (ORTE_PROC_IS_HNP) {
        ...
        OPAL_OUTPUT_VERBOSE((2, orte_routed_base_output,
                             "%s routed_rts: init routes for HNP job %s",
                             ORTE_NAME_PRINT(ORTE_PROC_MY_NAME),
                             ORTE_JOBID_PRINT(job)));
    ...
    {  /* MUST BE A PROC */
        ...
        if (NULL != ndat) {
            ...
            OPAL_OUTPUT_VERBOSE((1, orte_routed_base_output,
                                 "%s routed_rts: init routes w/non-NULL data",
                                 ORTE_NAME_PRINT(ORTE_PROC_MY_NAME)));
            ...
            if (ORTE_JOB_FAMILY(ORTE_PROC_MY_NAME->jobid) != ORTE_JOB_FAMILY(job)) {
                 ...
                 */
                OPAL_OUTPUT_VERBOSE((1, orte_routed_base_output,
                                     "%s routed_rts_init_routes: diff job family - sending update to %s",
                                     ORTE_NAME_PRINT(ORTE_PROC_MY_NAME),
                ...
                OPAL_OUTPUT_VERBOSE((1, orte_routed_base_output,
                                     "%s routed_rts_init_routes: ack recvd",
                                     ORTE_NAME_PRINT(ORTE_PROC_MY_NAME)));
        ...
        OPAL_OUTPUT_VERBOSE((1, orte_routed_base_output,
                             "%s routed_rts: init routes for proc job %s\n\thnp_uri %s\n\tdaemon uri %s",
                             ORTE_NAME_PRINT(ORTE_PROC_MY_NAME), ORTE_JOBID_PRINT(job),
                             (NULL == orte_process_info.my_hnp_uri) ? "NULL" : orte_process_info.my_hnp_uri,
                             (NULL == orte_process_info.my_daemon_uri) ? "NULL" : orte_process_info.my_daemon_uri));
        ...
        if (NULL == orte_process_info.my_daemon_uri) {
            ...
            opal_output(0, "%s ERROR: Failed to identify the local daemon's URI",
                        ORTE_NAME_PRINT(ORTE_PROC_MY_NAME));
            opal_output(0, "%s ERROR: This is a fatal condition when the rts router",
                        ORTE_NAME_PRINT(ORTE_PROC_MY_NAME));
...
static int route_lost(const orte_process_name_t *route)
{
    ...
    if (!orte_finalizing &&
        ...
        opal_output(0, "%s routed:rts: Connection to lifeline %s lost",
                    ORTE_NAME_PRINT(ORTE_PROC_MY_NAME),
...
static orte_vpid_t get_routing_tree(opal_list_t *children)
{
    ...
    /* for the HNP, the rts routing tree is direct to all known alive daemons */
    if (NULL != children) {
        if (NULL == (jdata = orte_get_job_data_object(ORTE_PROC_MY_NAME->jobid))) {
            ORTE_ERROR_LOG(ORTE_ERR_NOT_FOUND);
