...
int main(int argc, char *argv[])
{
    int rank, size;
    int c;
    int common, flag[NBREQ], ret;
    unsigned int seed = 1789;
    int ag_nb = 0, i;
    int *coll_ftbasic_debug_rank_may_fail;
    MPI_Request req[NBREQ];
    MPI_Status stat[NBREQ];
    int some_pending = 0;
    unsigned char completed[NBREQ];
    ...
    do {
        ...
        do {
            ...
            for(i = 0; i < NBREQ; i++) {
                flag[i] = rand() | common;
                if( verbose ) {
                    fprintf(stderr, "Agreement Nb %d: Rank %d/%d enters MPI_Comm_iagree with %08x\n", ag_nb+i, rank, size, flag[i]);
                }
                ret |= MPIX_Comm_iagree(MPI_COMM_WORLD, &flag[i], &req[i]);
            }

            memset(completed, 0, NBREQ * sizeof(char));
            do {
                some_pending = 0;
                ret = MPI_Waitall(NBREQ, req, stat);
                if( MPI_ERR_PENDING == ret )
                    some_pending = 1;
                for(i = 0; i < NBREQ; i++) {
                    if( !completed[i] ) {
                        if( MPI_ERR_PENDING == stat[i].MPI_ERROR ) {
                            some_pending = 1;
                            if( verbose ) {
                                fprintf(stderr, "Agreement Nb %d: Rank %d/%d leaves MPI_Comm_agree with %08x and ERR_PENDING\n", ag_nb+i, rank, size, flag[i]);
                            }
                            continue;
                        }
                        ret |= stat[i].MPI_ERROR;
                        if( verbose )
                            fprintf(stderr, "Agreement Nb %d: Rank %d/%d leaves MPI_Comm_agree with %08x and %d/%d\n", ag_nb+i, rank, size, flag[i], stat[i].MPI_ERROR, ret);
                        completed[i] = 1;
                    }
                }
            } while( some_pending );
            ...
            if( ret == MPI_SUCCESS ) {
                ...
                fprintf(stderr, "Agreement Nb %d to %d: Rank %d/%d is initial world is now Rank %d/%d in the alive processes\n",
                        ag_nb, ag_nb+i,
                        rank, size,
